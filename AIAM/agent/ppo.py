...
class PPO:
    def __init__(self, problem_name, p, opts):
    ...
    def rollout(self, problem, val_m, batch, do_sample=False, show_bar=False):
        ...
        bs, us, dim = batch['users'].size()
        batch['users'] = batch['users'].unsqueeze(1).repeat(1, val_m, 1, 1)
        ...
        if val_m > 1:
            for i in range(val_m):
                ...
                for aug in augments:
                    if aug == 'Rotate':
                        batch['users'][:, i] = rotate_tensor(batch['users'][:, i],
                        int(id_[0] * 4 + 1) * 90)
                    elif aug == 'Flip_x-y':
                        if int(id_[1] * 2 + 1) == 1:
                            data = batch['users'][:, i].clone()
                            batch['users'][:, i, :, 0] = data[:, :, 1]
                            batch['users'][:, i, :, 1] = data[:, :, 0]
                    elif aug == 'Flip_x_cor':
                        if int(id_[2] * 2 + 1) == 1:
                            batch['users'][:, i, :, 0] = 1 - batch['users'][:, i, :, 0]
                    elif aug == 'Flip_y_cor':
                        if int(id_[3] * 2 + 1) == 1:
                            batch['users'][:, i, :, 1] = 1 - batch['users'][:, i, :, 1]

        batch['users'] = batch['users'].view(-1, us, dim)

        bs, fs, dim = batch['facilities'].size()
        batch['facilities'] = batch['facilities'].unsqueeze(1).repeat(1, val_m, 1, 1)
        augments = ['Rotate', 'Flip_x-y', 'Flip_x_cor', 'Flip_y_cor']
        if val_m > 1:
            for i in range(val_m):
                random.shuffle(augments)
                ...
                for aug in augments:
                    if aug == 'Rotate':
                        batch['facilities'][:, i] = rotate_tensor(batch['facilities'][:, i],
                                                                   int(id_[0] * 4 + 1) * 90)
                    elif aug == 'Flip_x-y':
                        if int(id_[1] * 2 + 1) == 1:
                            data = batch['facilities'][:, i].clone()
                            batch['facilities'][:, i, :, 0] = data[:, :, 1]
                            batch['facilities'][:, i, :, 1] = data[:, :, 0]
                    elif aug == 'Flip_x_cor':
                        if int(id_[2] * 2 + 1) == 1:
                            batch['facilities'][:, i, :, 0] = 1 - batch['facilities'][:, i, :, 0]
                    elif aug == 'Flip_y_cor':
                        if int(id_[3] * 2 + 1) == 1:
                            batch['facilities'][:, i, :, 1] = 1 - batch['facilities'][:, i, :, 1]

        batch['facilities'] = batch['facilities'].view(-1, fs, dim)
        ...
        batch_users_feature, batch_facilities_feature = problem.input_feature_encoding(batch)
        bs, us, dim = batch_users_feature.size()
        _, fs, _ = batch_facilities_feature.size()

        exchange = None
        # action_record = [torch.zeros((bs, problem.size // 2)) for i in range(problem.size // 2)]
        action_record = [torch.zeros((batch_users_feature.size(0), problem.n_facilities)) for i in
                         range(problem.n_facilities * 2)]
        for t in tqdm(range(self.opts.T_max), disable=self.opts.no_progress_bar or not show_bar, desc='rollout',
                      bar_format='{l_bar}{bar:20}{r_bar}{bar:-20b}', position=0):
            # pass through model
            exchange = self.actor(problem,
                                  batch_users_feature,
                                  batch_facilities_feature,
                                  solutions,
                                  exchange,
                                  action_record,
        ...
        out = (obj[:, -1].reshape(bs, val_m).min(1)[0],  # batch_size, 1
               torch.stack(obj_history, 1)[:, :, 0].view(bs, val_m, -1).min(1)[0],  # batch_size, T   avg cost
               torch.stack(obj_history, 1)[:, :, -1].view(bs, val_m, -1).min(1)[0],  # batch_size, T  avg best cost
...
def train(rank, problem, agent, val_dataset, tb_logger):
    ...
    if opts.distributed:
        ...
        if torch.cuda.device_count() > 1:
            agent.actor = torch.nn.parallel.DistributedDataParallel(agent.actor,
                                                                    device_ids=[rank])
            if not opts.eval_only:
                agent.critic = torch.nn.parallel.DistributedDataParallel(agent.critic, device_ids=[rank])
        if not opts.no_tb and rank == 0:
            tb_logger = TbLogger(os.path.join(opts.log_dir, "{}_{}".format(opts.problem,
                                                                           opts.n_users), opts.run_name))
    else:
        for state in agent.optimizer.state.values():
            for k, v in state.items():
                if torch.is_tensor(v):
    ...
    for epoch in range(opts.epoch_start, opts.epoch_end):
        ...
        training_dataset = problem.make_dataset(n_users=opts.n_users, n_facilities=opts.n_facilities,
                                                p=opts.p, num_samples=opts.epoch_size)
        if opts.distributed:
...
def train_batch(
    ...
    batch_users_feature, batch_facilities_feature = problem.input_feature_encoding(batch)
    batch_users_feature = batch_users_feature.cuda() if opts.distributed \
        else move_to(batch_users_feature, opts.device)
    batch_facilities_feature = batch_facilities_feature.cuda() if opts.distributed \
        else move_to(batch_facilities_feature, opts.device)
    batch_size = batch_users_feature.size(0)

    exchange = move_to_cuda(torch.tensor([-1, -1]).repeat(batch_size, 1), rank) if opts.distributed \
        else move_to(torch.tensor([-1, -1]).repeat(batch_size, 1), opts.device)

    action_record = [torch.zeros((batch_users_feature.size(0), problem.n_facilities))
                     for i in range(problem.n_facilities * 2)]
    ...
    while t < T:
        ...
        for _k in range(K_epochs):
            ...
            # estimate return
            R = agent.critic(agent.actor(problem, batch_users_feature, batch_facilities_feature,
                                         solution, exchange, action_record, only_critic=True),
                             obj.view(obj.size(0), -1)[:, -1].unsqueeze(-1))[0]
            ...
            # ratios = torch.exp(logprobs[400:500] - old_logprobs[400:500].detach())
            #
            # # Finding Surrogate Loss:
            # advantages = Reward[400:500] - bl_val_detached[400:500]
            #
            ...
            #     baseline_loss = ((bl_val[400:500] - Reward[400:500]) ** 2).mean()
            #     old_value = bl_val.detach()[400:500]
            # else:
            #     vpredclipped = old_value + torch.clamp(bl_val[400:500] - old_value, - eps_clip, eps_clip)
            #     v_max = torch.max(((bl_val[400:500] - Reward[400:500]) ** 2), ((vpredclipped - Reward[400:500]) ** 2))
            #     baseline_loss = v_max.mean()
            #
            # # check K-L divergence
            # approx_kl_divergence = (.5 * (old_logprobs.detach()[400:500] - logprobs[400:500]) ** 2).mean().detach()
            ...
            if (not opts.no_tb) and rank == 0:
                if current_step % int(opts.log_step) == 0:
                    log_to_tb_train(tb_logger, agent, Reward, ratios, bl_val_detached, total_cost, grad_norms,
                                    memory.rewards, entropy, approx_kl_divergence,
